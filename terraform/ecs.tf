...
module "backend" {
  ...
  ecr_repository_uri           = "${data.aws_caller_identity.current.account_id}.dkr.ecr.${var.region}.amazonaws.com/orbit-backend"
  ...
  target_group_name_override   =  "orbit-be-${var.env}-tg"
  permissions_boundary_name    = "infra/i-dot-ai-${var.env}-orbit-perms-boundary-app"
  ...
  environment_variables = {
    ...
    "DOCKER_BUILDER_CONTAINER": "orbit",
  }
...
module "frontend" {
  ...
  source                       = "git::https://github.com/i-dot-ai/i-dot-ai-core-terraform-modules.git//modules/infrastructure/ecs?ref=v6.0.0-ecs"
  image_tag                    = var.image_tag
  ecr_repository_uri           = "${data.aws_caller_identity.current.account_id}.dkr.ecr.${var.region}.amazonaws.com/orbit-frontend"
  vpc_id                       = data.terraform_remote_state.vpc.outputs.vpc_id
  private_subnets              = data.terraform_remote_state.vpc.outputs.private_subnets
  host                         = local.host
  ...
  certificate_arn              = module.acm_certificate.arn
  target_group_name_override   = "orbit-fe-${var.env}-tg"
  permissions_boundary_name    = "infra/i-dot-ai-${var.env}-orbit-perms-boundary-app"
...
resource "aws_ecs_cluster_capacity_providers" "cluster" {
  ...
  capacity_providers = compact([
      ...
      module.backend.gpu_capacity_provider_name,
    ])
